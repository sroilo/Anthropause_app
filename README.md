...
**Developed by:**
Stephanie Roilo (stephanie.roilo@uni-bonn.de), TUD Dresden University of Technology, Germany & University of Bonn, Germany.
...
The Anthropause app is an open-source, interactive Shiny app that allows users to explore how COVID-19 lockdowns impacted primary biodiversity data collection across the globe. The app uses data from the [Global Biodiversity Information Facility](https://www.gbif.org) to quantify changes in the number of records collected during the lockdown compared to pre-pandemic levels. Moreover, the app visualises how the number of georeferenced human observations collected during the pandemic relates to the stringency of countries’ lockdown policies and the consequent changes in human mobility.
...
1.	A scatterplot in which each bubble represents a country, the size of the bubble reflects the number of GBIF records collected during the selected time period, and its colour reflects the mean stringency index. The countries are plotted along gradients in human mobility (the mean change in time spent at home on the x-axis and the mean change in visitors to park on the y-axis);
...
3.	A scatterplot in which each bubble represents a country, the size of the bubble reflects the number of GBIF records collected during the selected time period, and its colour reflects the mean percent change in park visitors. The countries are plotted against the change in GBIF records (x-axis) and the mean stringency index (y-axis).
...
Because retrieving the number of GBIF records for all countries takes some time, the app runs on a pre-compiled spreadsheet (‘Data_250_countries_Jan2025snapshot_20250109.csv’), based on the GBIF occurrence export dated January 1st, 2025 (DOI: https://doi.org/10.15468/dl.uzt932). The R script used to compile the dataset (‘Data_download.R’), as well as the two R scripts (‘ui.R’ and ‘server.R’) necessary to run the app locally, are open-source and accessible at https://github.com/sroilo/Anthropause_app.
...
# Acknowledgements
Many thanks to Meike Will, Jan Engler, and the [Chair of Computational Landscape Ecology](https://cle.geo.tu-dresden.de/) of the TUD Dresden University of Technology for constructive feedback during the development of this app. 
